...
export default function Navbar({ lang, toggleLang }: NavbarProps) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-[background-color,padding,border-color,box-shadow] duration-300 pt-safe ${isScrolled ? 'bg-[#050505]/95 shadow-[0_4px_30px_rgba(0,0,0,0.4)] border-b border-white/5 py-3 md:py-4 backdrop-blur-xl' : 'bg-transparent py-4 md:py-6 border-b border-transparent'}`}>
      
      {/* Scroll Progress Bar */}
      <div className="absolute bottom-0 left-0 h-[2px] bg-[#FF5500]/30 w-full">
        <motion.div 
          className="h-full bg-[#FF5500] shadow-[0_0_8px_rgba(255,85,0,0.8)]"
              ...
              return link.highlight
                ? (
                  <a
                    ...
                    href={link.href}
                    onClick={handleClick}
                    className="text-[14px] font-black text-white bg-[#FF5500] hover:bg-[#E04B00] px-4 py-1.5 rounded-full transition-all shadow-[0_0_12px_rgba(255,85,0,0.3)] hover:shadow-[0_0_20px_rgba(255,85,0,0.5)] cursor-pointer"
                  >
                    {link.name}
                  </a>
                ) : (
                  ...
                  >
                    {link.name}
                    <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-[#FF5500] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center" />
                  </a>
                );
            })}
          </motion.nav>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden overflow-hidden absolute top-full left-0 right-0 bg-[#050505] border-b border-white/10 shadow-2xl max-h-[80dvh] overflow-y-auto"
          >
